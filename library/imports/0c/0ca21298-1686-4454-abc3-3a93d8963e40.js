...
cc.Class({
  ...
  ctor: function ctor() {
    ...
    this.protoFiles = ["proto/baseproto", "proto/lobbyservice", "proto/gamebase", "proto/benz/gameservice", "proto/fruitMachine/gameservice", "proto/mayaMachine/gameservice", "proto/indiaMachine/gameservice", "proto/updown/gameservice", "proto/andeer/gameservice", "proto/baccarat3Patti/gameservice", "proto/horseRace/gameservice", "proto/lhd/gameservice", "proto/munda/gameservice", "proto/ssc/gameservice", "proto/tpGame/gameservice", "proto/rummy/gameservice", "proto/rocket/gameservice", "proto/zoo/gameservice", "proto/cricket/gameservice", "proto/zeus/gameservice"];
  ...
  onLoad: function onLoad() {
    ...
    CommonFun.getInstance().removeSidebar(); // Update节点部分
    ...
    this.lab_updateProgress = this.progressBar.node.getChildByName("NodeUpdateTips").getChildByName("lab_%").getComponent(cc.Label); // login节点部分
  ...
  start: function start() {
    ...
    if (cc.sys.isNative) {
      LoggerUtil.getInstance().log("H5: 11111111");
      CommonFun.getInstance().behaviorReporting(GlobalCfg.BEHAVIOR_TYPE.UPDATE_START);
      this.runUpdateProcess();
    } else if (cc.sys.isBrowser) {
      LoggerUtil.getInstance().log("H5: 22222222"); // H5 环境

      this.preloadMainH5();
    } else {
      LoggerUtil.getInstance().log("H5: 33333333"); // 其它环境，比如小程序之类
  ...
  comparisonFilesByMainManifestInfo: function comparisonFilesByMainManifestInfo(json) {
    ...
    for (var i = 1; i <= this.totalNumberOfRemoteMainFiles; i++) {
      ...
      ;
      this.simulationCheckFileAcount += 1; // 模拟校验文件数据的进度过程
