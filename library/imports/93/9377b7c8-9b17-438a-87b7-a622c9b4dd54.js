...
cc.Class({
  ...
  ctor: function ctor() {
    ...
    "Your cash is insufficient, Please recharge in time!"]; //投注额度数组
  ...
  onLoad: function onLoad() {
    ...
    this.lab_betAmount.string = this.betAmountArr[0]; //自动下注次数选择的展示

    this.node_tcBg.active = false; //总共赢的金额
  ...
  checkWebMsgError: function checkWebMsgError(webData, target) {
    ...
    if (msgId === "gameservice.login") {
      ...
      if (CommonFun.getInstance().isFreePlayerDirectedToFreeTP()) {
        if (result.result == 57) {
          CommonFun.getInstance().showDiversionFreeTP(function () {// SceneManager.getInstance().changeScene(SceneManager.getInstance().sceneType.SGJ, SceneManager.getInstance().sceneType.LOBBY);
          });
        } else {
  ...
  setCallNotify: function setCallNotify(notify) {
    ...
    this.playGameSound(zhuangClipName); //先扣除下注的金额
  ...
  startFruitAnim: function startFruitAnim() {
    ...
    for (var i = 0, len = this.node_fruitContentArr.length; i < len; i++) {
      ...
      var _loop = function _loop(k, lenk) {
        var item = children[k];
        item.repeat = 0;
        item.index = k;
        item.shu = i;
