...
cc.Class({
  ...
  onCustomEventMsg: function onCustomEventMsg(webData, target) {
    ...
    if (msgId == "GAME_ZOO_ROULETTEE_END") {
      ...
      self.zooAudioManager.playGameSound('showAnimal'); // 轮盘转动结束
      ...
      if (CommonFun.getInstance().isFreePlayerDirectedToFreeTP()) {
        if (result.result == 57) {
          CommonFun.getInstance().showDiversionFreeTP(function () {
            GameServerManager.send("gameservice.exit", "ExitReq", {}); // SceneManager.getInstance().changeScene(SceneManager.getInstance().sceneType.ZOO, SceneManager.getInstance().sceneType.LOBBY);
          });
        } else {
