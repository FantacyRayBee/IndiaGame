...
cc.Class({
  ...
  onDestroy: function onDestroy() {
    ...
    CommonFun.getInstance().behaviorReporting(GlobalCfg.BEHAVIOR_TYPE.EXIT_UPDOWN_GAME); // CommonFun.getInstance().releaseBundle("7up7downGame");
  },
  // ---------------------------------------------处理服务器的消息 Start----------------------------------------
  onEventMsg: function onEventMsg(webData, target) {
    ...
    if (msgId === "gameservice.login") {
      ...
      CommonFun.getInstance().hidProgress(); // self.userInfoCtrl && self.userInfoCtrl.setVipSiteState(false);
      ...
      if (CommonFun.getInstance().isFreePlayerDirectedToFreeTP()) {
        if (result.result == 57) {
          CommonFun.getInstance().showDiversionFreeTP(function () {
            GameServerManager.send("gameservice.exitgamereq", "ExitGameReq", {}); // SceneManager.getInstance().changeScene(SceneManager.getInstance().sceneType.SEVENUPDOWN, SceneManager.getInstance().sceneType.LOBBY);
          });
        } else {
          if (result.result == 19) {
            CommonFun.getInstance().showMsgBox("Your cash is insufficient, Please recharge in time！", "SHOP", function () {
  ...
  setGameEndInfo: function setGameEndInfo(notify) {
    ...
    this.gameresult = notify.GameResult;
    this.selfGameresult = notify.SelfGameResult; // this.userInfoCtrl.setCoin(notify.Gold);

    this.lab_tip.string = CommonFun.getInstance().showLabelLanguage("Billing...");
    this.win = notify.Win; // this.setGameEndSke(notify, true);       //摇出的两个色子的结果num1 num2
  ...
  settlementFeiGold: function settlementFeiGold(notify) {
    ...
    this.myBetCion = false; //VIP玩家连续三局未下注，踢下VIP座
  ...
  moveToPlayer: function moveToPlayer() {
    ...
    if (this.selfGameresult && Object.keys(this.selfGameresult).length !== 0 && this.selfGameresult.Score > 0) {
      (function () {
        //自己下注了，而且赢了
        var posSelf = cc.v2(-11, -216); // var selfCount = parseInt(this.selfGameresult.Score / 100);Object.keys(dataObject).length == 0

        _this6.userInfoCtrl.setWinNum(_this6.selfGameresult.Score);

        var selfWinJbArr = _this6.winGoldArr.splice(0, 20);
        ...
        var _loop2 = function _loop2(_i3, _len) {
          ...
          var palyersite = sortameresult[_i3].Site; // LoggerUtil.getInstance().log("!!!!!!!!!!!!!", palyersite, i);

          _this6.scheduleOnce(function () {
            for (var j = 0; j < _this6.vipPlayerScriptArr.length; j++) {
              var vipsite = _this6.vipPlayerScriptArr[j].getVipSite(); // LoggerUtil.getInstance().log("JJJJJJJJJJJJJJJ", vipsite);
      ...
      var _loop4 = function _loop4(_i4, _len2) {
        //此处的i为notify中的玩家数目
        var palyersite = sortameresult[_i4].Site;
        var afterGold = sortameresult[_i4].After;
        var score = sortameresult[_i4].Score; // LoggerUtil.getInstance().log("---------------!!!!!!!!!!!!!-------------", palyersite, i);
  ...
  setCallNotify: function setCallNotify(notify) {
    ...
    if (playerid === selfplayID) {
      //玩家自己
      this.roundBet = this.roundBet + Number(amount); // if(this.curRoundBetData.hasOwnProperty(call)){
      ...
      if (_script.getPlayerID() == null) {// LoggerUtil.getInstance().log("此时的座位为空！");
      } else {
        if (_script.getPlayerID() == playerid) {
          ...
          if (this.jbAudioState == true) {
            this.upDownAudioCtrl.playGameSound("otherCoin", false); // this.jbAudioState = false;
        ...
        } else {// LoggerUtil.getInstance().log("此时座位上的VIP不是之前下注的VIP玩家");
        }
      }
    }
  },
  ...
  setGameReadyStatus: function setGameReadyStatus(notify) {
    ...
    if (remaining > 15 && remaining < 20 && isFirstTime == true) {
      this.isPlayAudio = true; // this.myBetCion = true;
      ...
      if (time == 0) {// this.saizhongSke.setCompleteListener((trackEntry, loopCount) => {
        //     var name = trackEntry.animation.name;
        //     if (name == "kaishi") {
  ...
  setGameResultStatus: function setGameResultStatus(notify) {
    var self = this;
    var remaining = notify.Remaining; // this.gameresult = notify.GameResult;

    this.win = notify.Win;
    this.setXiaZhuBtn(false); // this.setResult();
  ...
  setGameConfig: function setGameConfig(notify) {
    ...
    this.reqVipplayerlist(); // this.userInfoCtrl.setFortSizeArr(notify.fortsize);
  },
  ...
  playFeiJinBiXiazhu: function playFeiJinBiXiazhu(count, areaType, fromPos, isSelf) {
    ...
    this.schedule(this.feiJbCallBack, 0.07, count - 1, 0); // for (let i = 0; i < count; i++) {
