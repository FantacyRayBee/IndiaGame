"use strict";
cc._RF.push(module, 'ed90banzVJA+peoEjYvYLqQ', 'cricketPlayerCtrl');
// cricketGame/scripts/cricketPlayerCtrl.js
...
cc.Class({
  ...
  start: function start() {
    ...
    this.btnGift.node.on('click', CommonFun.getInstance().debounce(function () {
      var seatId = _this.node.parent.getComponent('cricketSeatCtrl').seatId;
      if (GlobalCfg.ACT_SCENE_CTRL.seatManager.selfSeatId != -1) {
  ...
  loadHeadSp: function loadHeadSp(headUrl, realWidth, heaSprite) {
    ...
    if (headUrl && headUrl.length > 0) {
      cc.assetManager.loadRemote(headUrl, {
        ...
        if (!err && cc.isValid(_this2) && cc.isValid(heaSprite) && cc.isValid(heaSprite.spriteFrame)) {
  ...
  setUserCoinLabel: function setUserCoinLabel(diamond) {
    ...
    if (this.userId == GlobalCfg.USER_DATAS.userId) {
      ...
      ClientNotify.send(GlobalCfg.MSG_TYPE.clientMsg, {
        msgCode: "GAME_CRICKET_GIFT_SEND_COIN_UPDATE",
        msgData: {}
      });
    }
  },
