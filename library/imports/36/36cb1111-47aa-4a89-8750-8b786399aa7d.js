...
cc.Class({
  ...
  ctor: function ctor() {
    ...
    this.tuiChuLabel = ["Your game is not finished yet . If you wish to exit the table , you will lose your money . Do you want to leave table?", // 退出游戏
      ...
      if (CommonFun.getInstance().isFreePlayerDirectedToFreeTP()) {
        if (result.result == 57) {
          CommonFun.getInstance().showDiversionFreeTP(function () {
            GameServerManager.send("gameservice.exitroom", "ExitRoomReq", {}); // SceneManager.getInstance().changeScene(SceneManager.getInstance().sceneType.ANDAER, SceneManager.getInstance().sceneType.LOBBY);
          });
        } else {
          if (result.result == 19) {
            if (this.isTrialRoom) {
  ...
  setSceneInfo: function setSceneInfo(sceneInfo) {
    ...
    this.showMyBetBtns(false); // 设置庄家牌
  ...
  dealFirstBetStatusStartNotifyEvent: function dealFirstBetStatusStartNotifyEvent(notify) {
    ...
    this.scheduleOnce(function () {
      _this3.showMyBetBtns(true);

      for (var _i5 = 0, _len5 = _this3.playerNodeArr.length; _i5 < _len5; _i5++) {
        var _playerNode4 = _this3.playerNodeArr[_i5];
