...
cc.Class({
      ...
      if (CommonFun.getInstance().isFreePlayerDirectedToFreeTP()) {
        if (result.result == 57) {
          CommonFun.getInstance().showDiversionFreeTP(function () {
            GameServerManager.send("gameservice.exitroom", "ExitRoomReq", {}); // SceneManager.getInstance().changeScene(SceneManager.getInstance().sceneType.LHD, SceneManager.getInstance().sceneType.LOBBY);
          });
        } else {
  ...
  GameSceneRefresh: function GameSceneRefresh(sceneInfo, requester) {
    ...
    this.unscheduleAllCallbacks(); // 场景
  ...
  callnotify: function callnotify(notify) {
    ...
    var betCoinByBetTypes = function betCoinByBetTypes(type, pos) {
      var endNode = _this.betNodeArr[type]; // if(this.my_playerId != playerId){  // 自己在VIP座位上不显示投注金币动作
    ...
    this.lab_tieAllCion.string = tieAllCoin / 100; // 平民下注
  ...
  playerwinloseinfonotify: function playerwinloseinfonotify(notify) {
    ...
    if (ctrl) {// ctrl.diamond = this.myCoin;
    }
  },
  // 游戏结束
  ...
  UPGameAllBet: function UPGameAllBet() {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      endNode = this.betNodeArr[i]; // arr =  this.coinAllArr[i];
  ...
  loadSkeleData: function loadSkeleData() {
    ...
    if (this.assetBundle) {
      ...
      (function () {
        var self = _this4;
        skeleArr = ['lhdSke/vs_longhu', 'lhdSke/Win-dragon', "lhdSke/win-tiger", "lhdSke/win-tie", "lhdSke/Stop Betting", 'lhdSke/pai_z1'];
    ...
    var _loop = function _loop(k) {
      if (k != winorlose) {
        var _loop2 = function _loop2(j) {
          var node = _this5.coinAllArr[k][j];
          cc.tween(node).tag(1).delay((Math.random() * 0.2).toFixed(2)).to(0.2, {
            ...
            _this5.coinAllArr[k].splice(j, 1);
          }).start();
        };

        for (var j = _this5.coinAllArr[k].length - 1; j >= 0; j--) {
          _loop2(j);
        }
      }
    };
  ...
  playerWincCionAct: function playerWincCionAct() {
    ...
    var _loop3 = function _loop3(k) {
      ...
      var ctrl = _this6.getPlayerInfoByUserId(pos);
      ...
      var _loop4 = function _loop4(_i4) {
        ...
        if (node.name != "") {
          cc.tween(node).tag(1).delay((Math.random() / 3).toFixed(2)).to(0.3, {
            ...
            node.destroy();
            _this7.ske_kuang_long.active = false;
            _this7.ske_kuang_hu.active = false;
            _this7.ske_kuang_ping.active = false;
          }).start();
        }
      };
  ...
  showWinLosesAct: function showWinLosesAct(callback) {
    ...
    var pab_winorlose = cc.instantiate(this.pab_winorlose);
    var icon_t = pab_winorlose.getChildByName("icon_t").getComponent(cc.Sprite); // let lizi = pab_winorlose.getChildByName("lizi");
    // lizi.active = true;
  ...
  eventHide: function eventHide() {
    cc.game.on(cc.game.EVENT_HIDE, function () {
      ...
      this.ske_endWin.node.active = false;
      this.ske_vs_longhu.skeletonData = null;
      this.ske_vs_longhu.node.active = false; // cc.Tween.stopAll();
  ...
  startBetAim: function startBetAim() {
    ...
    this.ske_vs_longhu.setCompleteListener(function (trackEntry, loopCount) {
      ...
      if (name == "animation") {
        _this9.betStatus = true;

        _this9.repeatPreviousRound();

        _this9.showRuntime(14000);

        _this9.lhdCradCtrl.showCradStartAct(function () {
          _this9.ske_vs_longhu.skeletonData = null;
          _this9.ske_vs_longhu.node.active = false;
        });
      }
    });
    this.dragonChipArr = []; // 存放龙的区域的筹码
    ...
    if (spine) {
      ...
      this.ske_endWin.setCompleteListener(function (trackEntry, loopCount) {
        ...
        if (name == "Win-dragon-star" || name == "win-tiger-star" || name == "win-tie-star") {
          _this10.ske_endWin.addAnimation(0, animaNameArr[winType][2], false);
        } else if (name == "Win-dragon-loop" || name == "win-tiger-loop" || name == "win-tie-loop") {// this.ske_endWin.addAnimation(0, animaNameArr[winType][2], false); 
        } else if (name == "Win-dragon-out" || name == "win-tiger-out" || name == "win-tie-out") {
          _this10.showWinLosesAct(function () {
            _this10.ske_endWin.skeletonData = null;
            _this10.ske_endWin.node.active = false;
          });
        }
      });
    }
