...
cc.Class({
  ...
  onEventMsg: function onEventMsg(webData, target) {
    ...
    } else if (msgId == 'gameservice.bet') {// 下注通知
    ...
    } else if (msgId == 'gameservice.joinvip') {//vip入座
  ...
  onCustomEventMsg: function onCustomEventMsg(webData, target) {
    ...
    if (msgId == "GAME_CRICKET_ROULETTEE_END") {
      ...
      self.audioManager.playGameSound('settlementBGM'); // 轮盘转动结束
      ...
      if (CommonFun.getInstance().isFreePlayerDirectedToFreeTP()) {
        if (result.result == 57) {
          CommonFun.getInstance().showDiversionFreeTP(function () {
            GameServerManager.send("gameservice.exit", "ExitReq", {}); // SceneManager.getInstance().changeScene(SceneManager.getInstance().sceneType.CRICKET, SceneManager.getInstance().sceneType.LOBBY);
          });
        } else {
  ...
  */
  ...
  showEndRecordTween: function showEndRecordTween(ani, endNode) {
    ...
    var pos1 = endNode.getPosition(); // 游戏结束，生成节点至历史记录位置
