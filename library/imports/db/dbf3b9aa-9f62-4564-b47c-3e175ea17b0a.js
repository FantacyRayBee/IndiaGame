"use strict";
cc._RF.push(module, 'dbf3bmqn2JFZLR8PhdeoXsK', 'LobbyServerManager');
// Main/Script/Common/Net/LobbyServerManager.js

"use strict";

var customizeCloseEvent = {};
var LobbyServerManager = {
  socket: null,
  hearbeatTimer: null,
...
LobbyServerManager.startHeartBeat = function () {
  ...
  LobbyServerManager.hearbeatOutTimer = setTimeout(function () {
    if (LobbyServerManager.socket) {
      LobbyServerManager.socket.close();
    } else {
      LobbyServerManager.onClose(customizeCloseEvent, false);
...
LobbyServerManager.onReceive = function (evt) {
  ...
  if (msgTransPack.id != "lobbyservice.pingpang") {
    ...
    var protoMsgName = protoRootName + "." + LobbyServerManager.ProtoCfg[msgTransPack.id];
...
LobbyServerManager.checkDistributed = function (webData) {
  ...
  if (msgId === 'lobbyservice.kickout') {
    ...
    if (reason == 0) {
      ...
      // 封禁踢出
      CommonFun.getInstance().showMsgBox('Unable to log in, there is an exception with your account!', 'YES', function () {
        SceneManager.getInstance().changeScene(SceneManager.getInstance().sceneType.LOBBY, SceneManager.getInstance().sceneType.UPDATE);
      });
    }
    return false;
    ...
    if (Reflect.has(notify, 'success') == true) {
      if (notify.success == false) {
        var _obj = {};
        _obj['orderId'] = notify.orderId;
        _obj['amount'] = notify.amount;
    ...
    var _content = notify.content;
    var kind = notify.kind;
    CommonFun.getInstance().showMsgBox(_content, "YES", null, false, false, title);
    return false;
    ...
    GlobalCfg.USER_DATAS.userDiamond = _deposit + _winnings;
    GlobalCfg.USER_DATAS.deposit = _deposit;
    GlobalCfg.USER_DATAS.winnings = _winnings;
    GlobalCfg.USER_DATAS.bonus = _voucher;
    ClientNotify.send(GlobalCfg.MSG_TYPE.serverMsg, {
