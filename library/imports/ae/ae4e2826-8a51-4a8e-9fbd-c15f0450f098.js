...
cc.Class({
  ...
  onEventMsg: function onEventMsg(webData, target) {
    ...
    } else if (msgId == "gameservice.joinroomnotify") {//加入房间通知
    } else if (msgId == "gameservice.joinvipposnotify") {
      self.joinvipposnotify(notify);
    } else if (msgId == "gameservice.vipplayerlist") {//获取Vip列表
      ...
      if (CommonFun.getInstance().isFreePlayerDirectedToFreeTP()) {
        if (result.result == 57) {
          CommonFun.getInstance().showDiversionFreeTP(function () {
            GameServerManager.send("gameservice.outgame", "OutGameReq", {}); // SceneManager.getInstance().changeScene(SceneManager.getInstance().sceneType.HORSERACE, SceneManager.getInstance().sceneType.LOBBY);
          });
        } else {
  ...
  login: function login(notify) {
    ...
    this.myNodeCtrl.setUserData(userinfo); // this.sendReqCtrl.JoinRoomReq();
  ...
  endbetnotify: function endbetnotify(notify) {
    ...
    this.ske_start.addAnimation(0, "animation", false); // this.ske_start.setCompleteListener((trackEntry, loopCount) => {
  ...
  //移动记录球
  moveHistoryBall: function moveHistoryBall(id) {
    var _this9 = this;
  ...
  setHorseSpeedData: function setHorseSpeedData(TimeTemp) {
    ...
    if (TimeTemp) {
      ...
      for (var index = 1; index < this.horseAnimNodeArr.length; index++) {
        ...
        ske_horse.addAnimation(0, "animation", true); // }, Math.random())
      }
    } else {
  ...
  showTerminus: function showTerminus() {
    ...
    this.unscheduleAllCallbacks(); // this.horseRaceAudioCtrl.stopGameEffectByName();
    ...
    var _loop9 = function _loop9(_index12) {
      ...
      }).call(function () {// this.horseRaceAudioCtrl.stopGameEffectByName();
      }).start();
    };
  ...
  eventShow: function eventShow() {
    cc.game.on(cc.game.EVENT_SHOW, function () {
      ...
      this.unscheduleAllCallbacks(); // cc.Tween.stopAll();
  ...
  endGameAim: function endGameAim(selfScore) {
    ...
    this.ske_endWin.setCompleteListener(function (trackEntry, loopCount) {
      ...
      if (name == "star") {
        ...
        _this14.moveHistoryBall(winType);
      }
    });
