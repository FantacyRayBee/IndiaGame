...
if (packageChannel && packageChannel.indexOf("_") != -1) {
  ...
  switch (server) {
    case "0":
      // 测试服
      GlobalCfg.APP_VERSION = "0.1.2.7"; // GlobalCfg.APP_INFO_URL = `https://server.tpmass.com/AppInfo.json?time=${new Date().getTime()}`;
      // GlobalCfg.APP_CONFIG_URL = `https://server.tpmass.com/AppConfig.json?time=${new Date().getTime()}`;
    ...
    case "5":
      // 5服
      GlobalCfg.APP_VERSION = "5.0.0.34"; // GlobalCfg.APP_INFO_URL = `https://download.rax8.com/production/AppInfo.json`;
