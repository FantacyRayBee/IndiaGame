...
cc.Class({
    ...
    properties: {
        m_InnerScrollViews: [], //挂内嵌的ScrollView
        m_PlanDir: {
            //计划方向, 本次滑动的目的方向。根据刚开始滑动的方向决定 0为不限制方向, 1为横, -1为纵
            default: null,
            visible: false,
        },
        m_ScrollingInnerSv: {
    ...
    onLoad: function () {
        ...
        cc.systemEvent.on("add-netable-scrollview-inner", this.addNewInnerScrollView.bind(this), this);
    },

    addNewInnerScrollView(view) {
        if (!view) {
            return;
        }

        this.m_InnerScrollViews.push(view);
        view.setOuterScrollView(this);
    },
