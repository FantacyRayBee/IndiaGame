...
export namespace validator {
     ...
     * @returns
     */
    export function telNumber(telNumber: string): boolean {
    ...
    export function email(text: string): boolean {
        const reg = new RegExp("^[0-9a-z][_.0-9a-z-]{0,49}@([0-9a-z][0-9a-z-]{0,30}[0-9a-z].){1,4}[a-z]{2,4}$", "g");
        return reg.test(text);
    }
    ...
    export function isAuthorizated() {
        if (env.user.authorization?.length != 0) {
            return true;
        }
        return false;
    }
