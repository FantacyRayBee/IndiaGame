cc.Class({
    ...
    showResultAnima: function() {
        if (this.gameResult) {
            ...
            //奖励类型 (1:正常金币奖励, 2:免费次数奖励)
            let endedScore = this.gameResult.rewardtype == 2 ? this.gameResult.freePool/100 : totalMultiple * parseFloat(this.lab_betAmount.string) / 10 + startScore;
    ...
    sendCallReq: function() {
        if (GlobalCfg.IS_CLUB_MODE == 0 && GlobalCfg.USER_DATAS.isNotCharge == true && GlobalCfg.USER_DATAS.gamePattern == 0 && GlobalCfg.USER_DATAS.refuseUnpayHundred["minishuiguo"] == true){   //未曾充值
