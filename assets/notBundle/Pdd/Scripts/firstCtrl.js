cc.Class({
    ...
    initNode_1: function () {
        ...
        this.node_ske.setCompleteListener((trackEntry, loopCount) => {
            var name = trackEntry.animation.name;
            if (name == "chuxian") {
                this.node_ske.setAnimation(0, "chixu", true);
            } else if (name == "kai") {
                this.node_ske.setAnimation(0, "kai_chixu", true);
            } else if (name == "bianchang") {
                this.node_ske.setAnimation(0, "bianchang_chixu", true);
        ...
        if (btnName == "btn_open") {
            ...
            ResourcesBundle.load('NewPlan/Pdd/prefab/setBankInFirst', function (err, prefab) {
                if (err) {
                    ...
                    let nodeSetBank = cc.instantiate(prefab);
    ...
    movePlusToAdd: function () {
        ...
        this.finishMovePlus = cc.callFunc(function (target, value) {
            ...
            if (this.flyPlusCount == 0) {
                ...
                for (const key in stageGoldObj) {
                    if (Object.hasOwnProperty.call(stageGoldObj, key)) {
                        const element = stageGoldObj[key];
                        if (gold == key) {
                            stageGold = element;
                        }
                    }
            ...
            this.flyPlusCount++;
            let startGold = Number(this.lab_gold_2.string);

            this.danceLabel(stageGold, startGold);
        }, this, this.quota - 1);
        this.playPddSound("plus", false);
        let bezierTo_1 = cc.bezierTo(1, [cc.v2(280, -250), cc.v2(38, 50), cc.v2(8, 50)]);
        let spawn = cc.sequence(
            cc.spawn(
                bezierTo_1.easing(cc.easeSineInOut()),
    ...
    danceLabel: function (toNum, fromNum) {
        this.toNum = toNum;         // 跳动到达的金额
        this.startGold = fromNum;   // 跳动起始的金额
        let sc = cc.director.getScheduler();
    ...
    updateLabel: function (dt) {
        let end = Number(this.toNum);
        let start = Number(this.startGold);
        let curFrameRate = cc.game.getFrameRate();
        let cha = Math.ceil((end - start) / curFrameRate * 100) / 100;
        let num = Number(this.lab_gold_2.string);
        if (num + cha >= end) {
    ...
    stopDanceLabel: function () {
        ...
        switch (this.flyPlusCount) {
            case 1:
                this.showSecondTip();
                cc.tween(this.bg_ts_1)
                    .to(0.5, { position: cc.v2(0, -255) })
                    .start();
                break;
            case 2:     // show Node_3
                setTimeout(() => {
    ...
    showNode_3: function () {
        ...
        setTimeout(() => {
            this.showBianchangCallback();
        }, 300)
    },

    showBianchangCallback: function () {
        this.node_3.active = true;
        this.playPddSound("star", false);
        let spr_tx = this.bg_zj.getChildByName('tx_mask').getChildByName('tx').getComponent(cc.Sprite);
        let lab_name = this.bg_zj.getChildByName('lab_name').getComponent(cc.Label);
        ...
        let lab_gold = this.bg_zj.getChildByName('lab_gold').getComponent(cc.Label);        // 待领取的金额
        lab_gold.string = CommonFun.getInstance().formatCurrencyAmount(Number(this.unclaimed).toFixed(2));

        this.bg_zj.active = true;
        this.bg_zj.setPosition(cc.v2(0, 0));
        let self = this;
        cc.tween(this.bg_zj)
            .to(0.5, { position: cc.v2(0, 205) })
            .to(0.5, { scale: 0.6, opacity: 0 })
            .call(() => {
                this.bg_zj.active = false;
    ...
    addPddItem: function (prefab) {
        let pab_pddItem = cc.instantiate(prefab);
        pab_pddItem.getChildByName("icon_lucky").active = true;
        let spr_tx = pab_pddItem.getChildByName('tx_small').getComponent(cc.Sprite);
        this.loadHeadSp(GlobalCfg.USER_DATAS.userHeadimgurl, 36, spr_tx);
        let lab_name = pab_pddItem.getChildByName('lab_name').getComponent(cc.Label);
        lab_name.string = CommonFun.getInstance().getStrByLength(GlobalCfg.USER_DATAS.userName, 6);
        this.content.addChild(pab_pddItem, 1, "selfItem");
        for (let i = 0; i < this.content.childrenCount - 1; i++) {
        ...
        this.node_ske_star.setCompleteListener((trackEntry, loopCount) => {
            var name = trackEntry.animation.name;
            if (name == "chixu") {
        ...
        let lan_unclaimed = cc.find("ATTACHED_NODE_TREE/ATTACHED_NODE:root/ATTACHED_NODE:hengfu/lan_unclaimed", this.node_ske_hengfu.node);
        lan_unclaimed.getComponent(cc.Label).string = this.unclaimed;
        let lab_quota = cc.find("ATTACHED_NODE_TREE/ATTACHED_NODE:root/ATTACHED_NODE:hengfu/lab_quota", this.node_ske_hengfu.node);
        lab_quota.getComponent(cc.Label).string = CommonFun.getInstance().formatCurrencyAmount(Number(this.quota));
        lab_quota.parent.active = true;
        this.node_ske_hengfu.setCompleteListener((trackEntry, loopCount) => {
