cc.Class({
    ...
    start() {
        ...
        cc.game.on(cc.game.EVENT_HIDE, function () {
            ...
            for (let i = 0,len = this.playersNode.children.length; i < len; i++) {
                ...
                let otherUserCtrl = otherUserNode.getComponent("rummyOtherUserCtrl");
    ...
    initNode: function () {
        ...
        for (let i = 0,len = this.playersNode.children.length; i < len; i++) {
            ...
            let otherUserCtrl = otherUserNode.getComponent("rummyOtherUserCtrl");
            otherUserCtrl.setSeatId(i+1);                           //初始化座位号 1-5
            let btn_gift_other = otherUserNode.getChildByName("tx_k").getChildByName("btn_gift_other").getComponent(cc.Button);
        ...
        this.btn_gift, this.btn_close, this.btn_open, this.btn_cardListShow];
    ...
    btnClick: function (button) {
        ...
        if (btnName == "btn_tableInfo") {
            ...
            let otherUserNode = this.recursionFindParent(button.node,"otherNode");
            let otherUserCtrl = otherUserNode.getComponent("rummyOtherUserCtrl");
            let seatId = otherUserCtrl.getSeatId();
            let launchID = this.changeRelativeSeatIdToAbsolute(seatId);
    ...
    onEventMsg: function (webData, target) {
        ...
        if (msgId === "gameservice.enterroom") {                    // 进入房间
            ...
            for (let i = 0,len = self.playersNode.children.length; i < len; i++) {
                let otherUserNode = self.playersNode.children[i];
                let otherUserCtrl = otherUserNode.getComponent("rummyOtherUserCtrl");
                otherUserCtrl && otherUserCtrl.countDown(false);
            }
            ...
            if(selfID == notify.uid && (notify.reason == 2 || notify.reason == 0)){
                ...
                let otherUserCtrl = self.getOtherNodeCtrlByPlayerId(notify.uid);
                if(notify.reason == 0 || notify.reason == 2){
            ...
            for (let i = 0,len = self.playersNode.children.length; i < len; i++) {
                let otherNode = self.playersNode.children[i];
                let otherUserCtrl = otherNode.getComponent("rummyOtherUserCtrl");
                if (otherUserCtrl.isUserGame) {
                    self.TipsLab(8);
                    isShowTips = true;
                    break
                }
            ...
            if (self.userInfoCtrl.playerid == playerId) {
                ...
                let otherUserCtrl = self.getOtherNodeCtrlByPlayerId(playerId);
                otherUserCtrl.setCoin(after);
            }
            ...
            if(notify.seat !== self.selfAbsoluteSeatId){ 
                let otherUserCtrl = self.getOtherNodeCtrlBySeat(notify.seat);
                otherUserCtrl.gameDropOrFinalcards(notify,false);
            }else{                                          
                //自己的广播
            ...
            if(notify.seat !== self.selfAbsoluteSeatId){ //自己的广播，忽略
                let otherUserCtrl = self.getOtherNodeCtrlBySeat(notify.seat);
                otherUserCtrl.gameDropOrFinalcards(notify,true);
            }
    ...
    check_GameStartData: function (notify) {
        ...
        let firstCards = notify.firstPickCard;   //第一张牌比大小的
        ...
        for (let i = 0; i < firstCards.length; i++) {
            ...
            if (this.selfAbsoluteSeatId == i) {          //自己的牌，用来确认谁是庄家
                LoggerUtil.getInstance().log("自己的第一张牌的数组下标：",i);
                this.actionFirstPai(element, { scale: 0.6, position: cc.v2(0, 20) });
            } else {
                let otherUserCtrl = this.getOtherNodeCtrlBySeat(i);
                let toPos = otherUserCtrl.getPosition();
                let toPosChangeToWorld = this.selfCardNode.convertToNodeSpaceAR(this.node.convertToWorldSpaceAR(toPos));
                this.actionFirstPai(element, { scale: 0.6, position: toPosChangeToWorld });
            }
        }
        if (this.selfAbsoluteSeatId == firstPickWinSeat) {           //自己是庄家
    ...
    check_ChooseHuData: function (notify) {
        ...
        if (notify.huSeat == this.selfSeat) {         //自己胡牌
            ...
            let otherWinUserCtrl = this.getOtherNodeCtrlBySeat(notify.huSeat);
            ...
            let huSeatRelative = this.changeAbsoluteSeatIdToRelative(notify.huSeat);
            for (let i = 0,len = this.playersNode.children.length; i < len; i++) {
                let otherNode = this.playersNode.children[i];
                let otherNodeCtrl = otherNode.getComponent("rummyOtherUserCtrl");
    ...
    check_onGamecalcData: function (notify) {
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            if (seat == this.selfSeat) {
                ...
                // }
                let otherUserCtrl = this.getOtherNodeCtrlBySeat(seat);
                otherUserCtrl.gameover(notify, players[i].calc);
            }
        }
    },
    ...
    check_GameSceneData: function (notify) {
        ...
        for (let i = 0; i < playerList.length; i++) {
            ...
            if (GlobalCfg.USER_DATAS.userId == userinfo.displayName) {
                ...
                let otherUserCtrl = this.getOtherNodeCtrlBySeat(item.seat);
        ...
        switch (status) {
            ...
            case 3:                                     //发牌中
                ...
                for (let i = 0; i < playerList.length; i++) {
                    ...
                    if (GlobalCfg.USER_DATAS.userId == userinfo.displayName) {
                        ...
                        this.fapaiCallBack2 = () => {
                            this.controlButton(this.isSelfTurn, this.isPickOneCard, this.clickPaiArr.length);
                            let otherUserCtrl = this.getOtherNodeCtrlBySeat(curSeat);
            ...
            case 4:                                     //游戏中
                ...
                this.setRecorderCard(true)
                for (let i = 0; i < playerList.length; i++) {
                    let item = playerList[i];       // PlayerInfo
                    let userinfo = item.user;
                    ...
                    if (GlobalCfg.USER_DATAS.userId == userinfo.displayName) {     //自己
                        ...
                        // 此处需确保第一个数据为玩家自己
                        if(item.seat == curSeat){   //当前操作的玩家
                            let otherUserCtrl = this.getOtherNodeCtrlBySeat(item.seat);
                            otherUserCtrl.setUserInfo(userinfo, item.seat);
                            otherUserCtrl.countDown(true, item.simpleCountdown, item.extraCountdown);
                        } else {
                            let otherUserCtrl = this.getOtherNodeCtrlBySeat(item.seat);
                            otherUserCtrl.setUserInfo(userinfo, item.seat);
                            otherUserCtrl.countDown(false);
                ...
                for (let i = 0; i < playerList.length; i++) {
                    ...
                    if (GlobalCfg.USER_DATAS.userId == userinfo.displayName) {     //自己
                        ...
                        if (this.selfSeat == curSeat) {             //当前操作的玩家，即赢家
                            ...
                            tishi_finishCtrl.setDeClareContent(otherWinUserCtrl.getNickName() + " has declared" + "\n" + "Group your cards and declare");
                            tishi_finish.parent = this.node;
                            this.userInfoCtrl.countDown(true, item.simpleCountdown - 1, item.extraCountdown, true, item.playerStatus);
                        }
                    } else {
                        // this.otherUserCtrl.setUserInfo(userinfo, item.seat);
                        let otherUserCtrl = this.getOtherNodeCtrlBySeat(item.seat);
                        otherUserCtrl && otherUserCtrl.setUserInfo(userinfo, item.seat);
                        if(curSeat == item.seat){
                            otherUserCtrl.countDown(false);
                        } else {
                            LoggerUtil.getInstance().log(`报胡中，当前for循环的i为${i},simpleCountdown为，${item.simpleCountdown}额外时间为：${item.extraCountdown}`);
                            otherUserCtrl.countDown(true, item.simpleCountdown - 1, item.extraCountdown, true, item.playerStatus);
                        }
    ...
    check_newPlayerJoinData(notify) {
        ...
        if (userInfo.displayName == GlobalCfg.USER_DATAS.userId) {
            ...
            let otherUserCtrl = this.getOtherNodeCtrlBySeat(seatId);
    ...
    OutCard: function (notify) {
        ...
        if (notify.seat == this.selfSeat) {
            ...
            let otherUserCtrl = this.getOtherNodeCtrlBySeat(notify.nextSeat);
            otherUserCtrl.countDown(true, notify.simpleTimeout, notify.extraTimeout);

        } else {                //其他玩家出牌
            if(notify.nextSeat == this.selfSeat){       // 自己
                this.showLobbyUI("qiCrad")
                let paiNode = this.createPaiNode();
                let paiSprite = this.getPaiSpriteFrameByValue(notify.card);
                let otherUserCtrl = this.getOtherNodeCtrlBySeat(notify.seat);
                let fromPos = otherUserCtrl.getPosition();
                paiNode.getComponent(cc.Sprite).spriteFrame = paiSprite;
                paiNode.setPosition(fromPos);    //出牌玩家的位置， 需赋值
                paiNode.scale = 0.8;
                ...
                let curOtherUserCtrl = this.getOtherNodeCtrlBySeat(notify.seat);
                curOtherUserCtrl.countDown(false);
            }else{
                let paiNode = this.createPaiNode();
                let paiSprite = this.getPaiSpriteFrameByValue(notify.card);
                let curOtherUserCtrl = this.getOtherNodeCtrlBySeat(notify.seat);
                let fromPos = curOtherUserCtrl.getPosition();
                paiNode.getComponent(cc.Sprite).spriteFrame = paiSprite;
                paiNode.setPosition(fromPos);    //出牌玩家的位置， 需赋值
                paiNode.scale = 0.8;
                paiNode.parent = this.node;
    ...
    checkScore: function (paiValue) {
        ...
        if (score == 0) {
            ...
            CommonFun.getInstance().showMsgBox("Are you sure you want to declare?", "YES_NO", (code) => {
                // 发送结算消息
                this.finishReq(paiValue, this.getPaiValueArr());
    ...
    fapaiAction: function (notify, isSelf) {
        if (isSelf) {
            ...
            let otherUserCtrl = this.getOtherNodeCtrlBySeat(notify.seat);
            let toPostion = otherUserCtrl.getPosition();
            ...
            if (notify.side == 0) {
                ...
                cc.tween(paiNode)
                    .tag(1)
                    .to(0.2, { scale: 0.8, position: toPostion })
                    .call(() => {
                        setTimeout(() => {
                            this.onCardRemoved(paiNode);
                        }, 500)
    ...
    getLastPaiPosX() {
        ...
        if(this.paiGroupArr.length == 0){return 0}
        let tempArr = [];
        for (let i = 0, len = this.paiGroupArr.length; i < len; i++) {
    ...
    getPlayerInfoByUserId: function (userID) {
        ...
        let relativeSeatId = this.changeAbsoluteSeatIdToRelative(userID);
    ...
    getUserArrayNode:function(players){
        let arr = [];
        for (let i = 0; i < players.length; i++) {
            let item = players[i];
            let nodeCtrl = this.getOtherNodeCtrlBySeat(item.seat);
            let node = nodeCtrl && nodeCtrl.node;
            node && arr.push(node);
        }
        return arr
    },
    ...
    getOtherNodeCtrlBySeat: function(seat) {
        let relativeSeatId = this.changeAbsoluteSeatIdToRelative(seat);
        for (let i = 0,len = this.playersNode.children.length; i < len; i++) {
            let otherUserNode = this.playersNode.children[i];
    ...
    getOtherNodeCtrlByPlayerId: function(playerId) {
        for (let i = 0,len = this.playersNode.children.length; i < len; i++) {
            let otherUserNode = this.playersNode.children[i];
            let rummyOtherUserCtrl = otherUserNode.getComponent("rummyOtherUserCtrl");
            let otherPlayerId = rummyOtherUserCtrl.getPlayerId();
     ...
     * @returns {String}
     */
    recursionFindParent: function(parentNode,nodeName) {
