cc.Class({
    ...
    setGameOverPlayerInfo: function (data, mainCtrl, selfCtrl, otherCtrl) {
    ...
    setPlayerInfo(playerlist, isDrop, iszhahu) {
        for (let i = 0; i < playerlist.length; i++) {
            ...
            if (player.seat == this.winSeat) {    //获胜者,iszhahu = true 时，为输家
                let winerChilds = this.getUserNodeChild(this.user_winer);
                let selfSeat = this.mainCtrl.selfSeat;
                ...
                let loseChilds = this.getUserNodeChild(this.user_loser);
                let selfSeat = this.mainCtrl.selfSeat;
                let winCoin = player.afterCalc;
                if (selfSeat == player.seat) {
