    ...
    btnClick: function (button) {
        ...
        if (btnName === "btn_close") {
            ...
            if (btnName === "btn_collect") {
                let url = GlobalCfg.HTTP_SERVER + "/v1/voucher/receivevoucherbet";
                CommonFun.getInstance().httpGet(url, (strInfo) => {
                    if (strInfo.result == 0) {
                        CommonFun.getInstance().showRewardsTips([{ id: 10, amount: GlobalCfg.USER_DATAS.undraw / 100 }]);
                        ...
                        ClientNotify.send(GlobalCfg.MSG_TYPE.clientMsg, { msgCode: GlobalCfg.CLIENT_MSG_ID.GET_BOUND_UNDRAW, msgData: {} });
                        if (CommonFun.getInstance().isValidForScr(this)) {
                            this.lab_Rs.string = strInfo.data.after_bet;
