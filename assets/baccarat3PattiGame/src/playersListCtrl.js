...
cc.Class({
    ...
    // 实例化玩家列表
    setPlayerDate:function(notify){
        this.isONE++
        let playerlist = notify.list;
        this.allcount = notify.total;
        this.content.removeAllChildren();
        for (let i = 0; i < playerlist.length; i++) {
            let pab_player = cc.instantiate(this.pab_player);
            let ctrl = pab_player.getComponent("PlayerItemCtrl")
    ...
    clickBtn:function(button){
        ...
        if (btnName == "btn_Prev") {
            ...
            if( this.page <= 1) {
                ...
                GlobalCfg.ACT_SCENE_CTRL.gameServiceSendCtrl.PlayerListReq(1,12);
            }else {
                GlobalCfg.ACT_SCENE_CTRL.gameServiceSendCtrl.PlayerListReq(this.page,12);
                this.lab_page.string = this.page + "/" + this.allPage
                this.btn_next.target.color = new cc.color(255,255,255,255)
                this.btn_next.interactable = true;
                this.btn_next.enableAutoGrayEffect = false;
