cc.Class({
    ...
    btnClick: function () {
        ...
        let act = this.upCtrl.hasDown == false ? 1 : 3;
    ...
    checkSelfIsVip:function(){
        let playerid = GlobalCfg.ACT_SCENE_CTRL.userInfoCtrl.getPlayerid();
    ...
    setVipPlayerInfo: function (data) {
    ...
    setWinNum: function (num) {
        if (num <= 0 || this.vipSiteState == true) { return };
    ...
    // 下注
    vipBetting: function (count, areaType, mainScript) {
        this.roundCount = 0;
        let pos = this.getVipPlayerPos();
        let betCallBack = ()=>{
            let jbnode = mainScript.createJbNode();
            ...
            cc.tween(jbnode)
                .to(0.25, { scale: 1, position: cc.v2(posx, posy) })
                .start();
        }
        this.schedule(betCallBack, 0.05, count - 1, 0);
    },

    winGoldMoveAnim:function(count, vipWinJbArr, mainScript){
        let pos = this.getVipPlayerPos();
        for (let i = vipWinJbArr.length - 1; i >= 0; i--) {
            let tempIndex = i;
            cc.tween(vipWinJbArr[tempIndex])
