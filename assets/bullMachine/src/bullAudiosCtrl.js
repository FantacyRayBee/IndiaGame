            ...
            CommonFun.getInstance().loadBundle('jokerMachine', (bundle) => {
                bundle.load(`audios/${audioClipName}`, cc.AudioClip, (err, audioClip) => {
                    if (!err) {
                        ...
                        LoggerUtil.getInstance().error(`加载jokerMachine-${audioClipName}异常1: ${JSON.stringify(err)}`);
                    };
                });
            }, (err) => {
                LoggerUtil.getInstance().error(`加载jokerMachine-${audioClipName}异常: ${JSON.stringify(err)}`);
            });
        };
    },
