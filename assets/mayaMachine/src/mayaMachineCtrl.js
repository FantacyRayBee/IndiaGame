cc.Class({
    ...
    startSlotsAnim: function() {
        ...
        this.isRunningMayaAnim = true;
        let isFast = this.toggle_fast.isChecked;
        this.kRate = isFast ? 0.45 : 1.1;
        let mayaContentTime = 0.1 * this.kRate;
        for (let i = 0, len = this.node_mayaContentArr.length; i < len; i++) {
            let children = this.node_mayaContentArr[i].children;
    ...
    checkAnimFinish: function() {
        this.finishedMayaItemNum += 1;
        if (this.finishedMayaItemNum == 20) {
            this.finishedMayaItemNum = 0;
    ...
    showResultAnima: function() {
        ...
        // ✅ 关键点：立即开始金币滚动（和中奖连线同时进行）
        this.runChangeTotalWinScore(startScore, endedScore, freeCount);

        // 把 bigwin 相关数据传给 showSpinResult，
        // 中奖线播完后再决定是否弹 bigwin，不影响自动 spin 流程
    ...
    showSpinResult: function(totalMultiple, bigWinData) {
        ...
        // 2. 等中奖线动画结束（isRunningMayaAnim=false）再往后走
        let startNextSpin = () => {
            if (this.isRunningMayaAnim) {
                return; // 中奖线动画还在播，再等等
            };
            ...
            // 封装：bigwin 弹窗结束后，继续“免费局/自动 spin/恢复按钮”的逻辑
            let continueFlow = () => {
            ...
            for (let i = 0, len = this.gameResult.xiannum.length; i < len; i++) {
                ...
                if (xiannumLen >= 3) {
                    for (let j = 0; j < xiannumNum; j++) {
                        ...
                        let shu4TypeArr = [];
                        if (xiannumLen >= 5) {
                            let shu4 = this.node_mayaContentArr[4];
                            for (let j = 0; j < 3; j++) {
    ...
    sendCallReq: function() {
        if (GlobalCfg.USER_DATAS.isNotCharge == true && GlobalCfg.USER_DATAS.gamePattern == 0 && GlobalCfg.USER_DATAS.refuseUnpayHundred["minimaya"] == true){   //未曾充值
