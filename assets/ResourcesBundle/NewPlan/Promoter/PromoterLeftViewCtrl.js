cc.Class({
    ...
    setInviteLeftViewLanguage: function () {
        ...
        let historyTipsStr = "";
        let historyOutPutTipsStr = "";
        let historyBonusTipsStr = "";
        let historyTimeStr = "";
        let historyOutPutStr = "";
        let historyInvitedStr = "";
    ...
    showLeftViewByTypeStr: function (typeStr) {
        this.curTypeStr = typeStr;
    },

    sendTeamRecordReq: function () {
        let httpUrl = GlobalCfg.HTTP_SERVER + "/v1/promoter/teamplayerincomerank";
        CommonFun.getInstance().httpGet(httpUrl, (msg) => {
            if (msg.result == 0) {
                if (msg.data && msg.data.rank && CommonFun.getInstance().isValidForScr(this)) {
                    this.setTeamRecord(msg.data.rank);
                };
            }
            else {
    ...
    sendHistoryRecordReq: function () {
        let httpUrl = GlobalCfg.HTTP_SERVER + "/v1/promoter/income/record";
        CommonFun.getInstance().httpGet(httpUrl, (msg) => {
            if (msg.result == 0) {
                if (msg.data && CommonFun.getInstance().isValidForScr(this)) {
                    this.setHistoryRecord(msg.data);
                };
            }
            else {
                CommonFun.getInstance().showTips(msg.msg);
    ...
    setHistoryRecord: function (data) {
        if (!data) {
            return;
        };
        ...
        if (data["day_record"]) {
            for (let i = 0, len = data["day_record"].length; i < len; i++) {
                if (i >= 7) {
                    return;
                };
                const element = data["day_record"][i];
