cc.Class({
    ...
    // 发送表情或消息
    ShortMessageReq: function (msgType, name) {
    ...
    initLocalBtn: function () {
        ...
        for (let i = 0; i < this.togg_btnArr.length; i++) {
            let tog = this.togg_btnArr[i];
            tog.node.on('click', this.toggleClick, this);
        }

        for (let k = 0; k < this.btnChildrenQPH.length; k++) {
            let btnQPH = this.btnChildrenQPH[k].getComponent(cc.Button);
            btnQPH.node.on('click', this.btnClickQPH, this);

        }

        for (let j = 0; j < this.btnChildrenTP.length; j++) {
            let btnTP = this.btnChildrenTP[j].getComponent(cc.Button);
    ...
    showWords: function () {
        for (let k = 0; k < this.btnChildrenQPH.length; k++) {
            let lab = this.btnChildrenQPH[k].getComponent(cc.Button).target.getChildByName("Label")
            let str = lab.getComponent(cc.Label);
            str.string = zjhQiaoPiHau[k][language]
        }
    },

    setPlayerSeat: function (msgid) {
        this.msgid = msgid
    },
