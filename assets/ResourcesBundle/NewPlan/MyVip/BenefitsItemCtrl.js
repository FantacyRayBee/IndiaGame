cc.Class({
    ...
    setBenifitsTimeForDay: function(diffTime) {
        let downTimer = () => {
            diffTime -= 1;
            if (diffTime < 0) {
    ...
    setBenifitsTimeForWeekOrMonth: function(endTime) {
        let dateObj = new Date(endTime * 1000);
 
        // 获取年、月、日
        let year = dateObj.getFullYear();
        let month = dateObj.getMonth() + 1; // JavaScript中月份从0开始计数，所以加上1
    ...
    setBenifitsItemData: function(type, data) {
        if (type <= 0 || type > 11) {
        ...
        if (GlobalCfg.USER_DATAS.userVip.level == data.level) {
            ...
            if (type == 5) {
                if (timestamp < GlobalCfg.USER_DATAS.userVip.next_day_take_time) {
                    ...
                    this.btn_icon.node.on("click", CommonFun.getInstance().debounce(() => {
                        ClientNotify.send(GlobalCfg.MSG_TYPE.clientMsg, {msgCode: GlobalCfg.CLIENT_MSG_ID.VIP_TAKE_WELFARE, msgData: {type:  "day"}});
                    }, 3), this);
                };
                ...
                if (timestamp < GlobalCfg.USER_DATAS.userVip.next_week_take_time) {
                    ...
                    this.btn_icon.node.on("click", CommonFun.getInstance().debounce(() => {
                        ClientNotify.send(GlobalCfg.MSG_TYPE.clientMsg, {msgCode: GlobalCfg.CLIENT_MSG_ID.VIP_TAKE_WELFARE, msgData: {type:  "week"}});
                    }, 3), this);
                };
                this.lab_tips2.node.active = true;
            }
                ...
                if (timestamp < GlobalCfg.USER_DATAS.userVip.next_month_take_time) {
                    ...
                    this.btn_icon.node.on("click", CommonFun.getInstance().debounce(() => {
                        ClientNotify.send(GlobalCfg.MSG_TYPE.clientMsg, {msgCode: GlobalCfg.CLIENT_MSG_ID.VIP_TAKE_WELFARE, msgData: {type:  "month"}});
                    }, 3), this);
                };
                this.lab_tips2.node.active = true;
            }
